...
def webhook(request, secret=None):
    if not get_system_setting('enable_openproject'):
        logger.debug('ignoring incoming webhook as OpenProject is disabled.')
        raise Http404('OpenProject disabled')
    elif not get_system_setting('enable_openproject_web_hook'):
        logger.debug('ignoring incoming webhook as OpenProject Webhook is disabled.')
        raise Http404('OpenProject Webhook disabled')
    elif not get_system_setting('disable_openproject_webhook_secret'):
        if not get_system_setting('openproject_webhook_secret'):
            logger.warning('ignoring incoming webhook as OpenProject Webhook secret is empty in Defect Dojo system settings.')
            raise PermissionDenied('OpenProject Webhook secret cannot be empty')
        if secret != get_system_setting('openproject_webhook_secret'):
            logger.warning('invalid secret provided to OpenProject Webhook')
            raise PermissionDenied('invalid or no secret provided to OpenProject Webhook')
    ...
    if request.method == 'POST':
        try:
            ...
            if isinstance(e, Http404):
                logger.warning('404 error processing OpenProject webhook')
            else:
                logger.exception(e)

            try:
                logger.debug('openproject_webhook_body_parsed:')
                logger.debug(json.dumps(parsed, indent=4))
            except:
                logger.debug('openproject_webhook_body:')
...
@user_is_configuration_authorized('dojo.add_openproject_instance')
def express_new_openproject(request):
    if request.method == 'POST':
        opform = ExpressOpenProjectForm(request.POST, instance=OpenProject_Instance())
        if opform.is_valid():
            op_server = opform.cleaned_data.get('url').rstrip('/')
            op_username = opform.cleaned_data.get('username')
            op_password = opform.cleaned_data.get('password')

            try:
                op = openproject_helper.get_openproject_connection_raw(op_server, op_username, op_password)
            except Exception as e:
                ...
                return render(request, 'dojo/express_new_openproject.html',
                                        {'opform': opform})
            ...
            op_instance = OpenProject_Instance(username=op_username,
                                    ...
                                    finding_text='',
                                    default_issue_type=opform.cleaned_data.get('default_issue_type'))
            op_instance.save()
            messages.add_message(request,
                                    messages.SUCCESS,
                                    'OpenProject Configuration Successfully Created.',
                                    extra_tags='alert-success')
            create_notification(event='other',
                                title='New addition of OpenProject: %s' % opform.cleaned_data.get('configuration_name'),
                                description='OpenProject "%s" was added by %s' %
                                            (opform.cleaned_data.get('configuration_name'), request.user),
                                url=request.build_absolute_uri(reverse('openproject')),
                                )
            return HttpResponseRedirect(reverse('openproject', ))
    else:
        opform = ExpressOpenProjectForm()
        add_breadcrumb(title="New OpenProject Configuration (Express)", top_level=False, request=request)
    return render(request, 'dojo/express_new_openproject.html',
                  {'opform': opform})


@user_is_configuration_authorized('dojo.add_openproject_instance')
def new_openproject(request):
    if request.method == 'POST':
        opform = OpenProjectForm(request.POST, instance=OpenProject_Instance())
        if opform.is_valid():
            op_server = opform.cleaned_data.get('url').rstrip('/')
            op_username = opform.cleaned_data.get('username')
            op_password = opform.cleaned_data.get('password')

            logger.debug('calling get_openproject_connection_raw')
            op_conn = openproject_helper.get_openproject_connection_raw(op_server, op_username, op_password)

            new_op = opform.save(commit=False)
            new_op.url = op_server
            new_op.save()
            messages.add_message(request,
                                    messages.SUCCESS,
                                    'OpenProject Configuration Successfully Created.',
            ...
            logger.error('opform.errors: %s', opform.errors)
    else:
        opform = OpenProjectForm()
        add_breadcrumb(title="New OpenProject Configuration", top_level=False, request=request)
    return render(request, 'dojo/new_openproject.html',
                  {'opform': opform})


@user_is_configuration_authorized('dojo.change_openproject_instance')
def edit_openproject(request, opid):
    op = OpenProject_Instance.objects.get(pk=opid)
    op_password_from_db = op.password
    if request.method == 'POST':
        opform = OpenProjectForm(request.POST, instance=op)
        if opform.is_valid():
            op_server = opform.cleaned_data.get('url').rstrip('/')
            op_username = opform.cleaned_data.get('username')

            if opform.cleaned_data.get('password'):
                op_password = opform.cleaned_data.get('password')
            else:
                # on edit the password is optional
                op_password = op_password_from_db
            ...
            new_op.url = op_server
            # on edit the password is optional
            new_op.password = op_password
            new_op.save()
            messages.add_message(request,
                                    messages.SUCCESS,
                                    'OpenProject Configuration Successfully Saved.',
                                    extra_tags='alert-success')
            create_notification(event='other',
                                title='Edit of OpenProject: %s' % opform.cleaned_data.get('configuration_name'),
                                description='OpenProject "%s" was edited by %s' %
                                            (opform.cleaned_data.get('configuration_name'), request.user),
                                url=request.build_absolute_uri(reverse('openproject')),
                                )
            return HttpResponseRedirect(reverse('openproject', ))
        ...
        opform = OpenProjectForm(instance=op)
        add_breadcrumb(title="Edit OpenProject Configuration", top_level=False, request=request)

    return render(request,
                  'dojo/edit_openproject.html',
                  {
                      'opform': opform,
                  })


@user_is_configuration_authorized('dojo.view_openproject_instance')
def openproject(request):
    openproject_instances = OpenProject_Instance.objects.all()
    add_breadcrumb(title="OpenProject List", top_level=not len(request.GET), request=request)
    return render(request,
                  'dojo/openproject.html',
                  {'openproject_instances': openproject_instances,
                   })


@user_is_configuration_authorized('dojo.delete_openproject_instance')
def delete_openproject(request, tid):
    op_instance = get_object_or_404(OpenProject_Instance, pk=tid)
    # eng = test.engagement
    # TODO Make Form
    form = DeleteOpenProjectInstanceForm(instance=op_instance)

    if request.method == 'POST':
        if 'id' in request.POST and str(op_instance.id) == request.POST['id']:
            form = DeleteOpenProjectInstanceForm(request.POST, instance=op_instance)
            if form.is_valid():
                try:
                    op_instance.delete()
                    messages.add_message(request,
                                        messages.SUCCESS,
                                        'OpenProject Conf and relationships removed.',
                                        extra_tags='alert-success')
                    create_notification(event='other',
                                        title='Deletion of OpenProject: %s' % op_instance.configuration_name,
                                        description='OpenProject "%s" was deleted by %s' % (op_instance.configuration_name, request.user),
                                        url=request.build_absolute_uri(reverse('openproject')),
                                        )
                    return HttpResponseRedirect(reverse('openproject'))
                except Exception as e:
                    add_error_message_to_response('Unable to delete OpenProject Instance, probably because it is used by OpenProject Issues: %s' % str(e))

    collector = NestedObjects(using=DEFAULT_DB_ALIAS)
    collector.collect([op_instance])
    rels = collector.nested()

    add_breadcrumb(title="Delete", top_level=False, request=request)
    return render(request, 'dojo/delete_openproject.html',
                  {'inst': op_instance,
                   'form': form,
                   'rels': rels,
